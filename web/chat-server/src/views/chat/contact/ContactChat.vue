...
export default {
  ...
  setup() {
    ...
    const sendMessage = () => {
      ...
      safeSocketSend(JSON.stringify(chatMessageRequest));
      data.chatMessage = "";
      scrollToBottom();
    };
    ...
    const sendFileMessage = async (fileUrl) => {
      ...
      safeSocketSend(JSON.stringify(chatFileMessageRequest));
      scrollToBottom();
    };
    ...
    const sendAvatarMessage = (avatarUrl) => {
      ...
      safeSocketSend(JSON.stringify(chatAvatarMessageRequest));
      scrollToBottom();
    };
    ...
    const createRtcPeerConnection = () => {
      ...
      data.rtcPeerConn.onicecandidate = (event) => {
        if (event.candidate) {
          ...
          safeSocketSend(JSON.stringify(rtcMessageRequest));
        }
      };
    ...
    // 统一的安全发送与自动重连
    const safeSocketSend = async (payload) => {
      try {
        if (!store.state.socket || store.state.socket.readyState !== WebSocket.OPEN) {
          const wsUrl = store.state.wsUrl + "/ws?client_id=" + data.userInfo.uuid;
          try {
            store.state.socket = new WebSocket(wsUrl);
          } catch (err) {
            ElMessage.error("实时连接初始化失败，请稍后重试");
            console.error(err);
            return;
          }
          await new Promise((resolve, reject) => {
            const onOpen = () => {
              store.state.socket.removeEventListener("open", onOpen);
              store.state.socket.removeEventListener("error", onError);
              resolve();
            };
            const onError = (e) => {
              store.state.socket.removeEventListener("open", onOpen);
            ...
            store.state.socket.addEventListener("open", onOpen);
            store.state.socket.addEventListener("error", onError);
          }).catch((e) => {
            ElMessage.error("实时连接未建立，消息发送失败");
            console.error(e);
            return;
          });
        }
        store.state.socket.send(payload);
      } catch (e) {
        ElMessage.error("发送失败：连接异常或已断开");
        console.error(e);
      }
    };

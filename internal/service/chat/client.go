...
// WebSocket 心跳参数
const (
	// pingPeriod 为服务端向客户端发送 Ping 的间隔；须小于 pongWait。
	pingPeriod = 30 * time.Second
	// pongWait 为等待客户端回 Pong 的最大时长；超时则认为连接已断开。
	pongWait = 60 * time.Second
	// writeWait 为一次 WebSocket 写操作的超时时长。
...
func (c *Client) Read() {
	...
	// 设置 Pong Handler：每次收到 Pong 刷新读超时，维持长连接活跃
...
// Write 从 SendBack 通道读取消息并通过 WebSocket 发送给客户端。
// 同时在独立 goroutine 中定时发送 Ping，维持心跳。
func (c *Client) Write() {
	zlog.Info("ws write goroutine start")

	// 心跳 Ping goroutine：每隔 pingPeriod 向客户端发一次 Ping
	...
	go func() {
		for range ticker.C {
			...
			if err := c.Conn.WriteMessage(websocket.PingMessage, nil); err != nil {
				zlog.Error("ws ping error: " + err.Error())
				return
			}
		}
	}()

...
import (
	...
	"kama_chat_server/internal/service/pool"
		...
		case client := <-s.Login:
			{
				...
				zlog.Debug(fmt.Sprintf("欢迎来到kama聊天服务器，亲爱的用户%s（当前在线: %d）\n",
					client.Uuid, GetActiveConnections()))
		...
		case client := <-s.Logout:
			{
				...
				zlog.Info(fmt.Sprintf("用户%s退出登录（当前在线: %d）\n", client.Uuid, GetActiveConnections()))
		...
		case data := <-s.Transmit:
			{
				...
				chatMessageReq.ServerSeq = sequence.GlobalSequencer.Next(chatMessageReq.SessionId)
				if chatMessageReq.TraceId == "" {
					chatMessageReq.TraceId = sequence.GlobalSequencer.TraceID(chatMessageReq.SessionId, chatMessageReq.SendId, chatMessageReq.ServerSeq)
				}
				...
				if chatMessageReq.Type == message_type_enum.Text {
					// ===== AI消息处理 =====
					// 检查是否是@ai消息
					aiHandler := ai.GetMessageHandler()
					if aiHandler.ShouldHandleByAI(chatMessageReq.Content) {
						zlog.Info(fmt.Sprintf("检测到AI消息: user=%s, content=%s", chatMessageReq.SendId, chatMessageReq.Content))
						...
						userMessage := model.Message{
							...
							ReceiveId:  ai.GetAIUserUUID(), // 发给AI
							FileSize:   "0B",
							FileType:   "",
							FileName:   "",
						...
						if res := dao.GormDB.Create(&userMessage); res.Error != nil {
							zlog.Error(res.Error.Error())
						}

						// 异步处理AI回复：通过协程池隔离大模型耗时操作，避免阻塞主调度循环
						capturedReq := chatMessageReq // 值拷贝，防止闭包捕获同一变量
						capturedUUID := userMessage.Uuid
						submitErr := pool.AIWorkerPool.Submit(func() {
							ctx := context.Background()
							aiResp, err := aiHandler.HandleAIMessage(ctx, &capturedReq)
							connMetrics.OnAIRequest(err != nil)
							if err != nil {
								zlog.Error("AI消息处理失败: " + err.Error())
								return
							}

							// 构建AI回复消息并推送给发送方
							jsonMessage, err := json.Marshal(aiResp)
							if err != nil {
								zlog.Error(err.Error())
								return
							}
							...
							s.mutex.Lock()
							if sendClient, ok := s.Clients[capturedReq.SendId]; ok {
								sendClient.SendBack <- messageBack
								connMetrics.OnMessageSent()
							}
							s.mutex.Unlock()

							zlog.Info(fmt.Sprintf("AI回复已推送给用户: %s, pool_stats: %+v",
								capturedReq.SendId, pool.AIWorkerPool.Stats()))
						})
						if submitErr != nil {
							// 协程池满时降级：直接裸 goroutine 兜底，保证消息不丢失
							zlog.Error("AI worker pool full, fallback to goroutine: " + submitErr.Error())
							capturedReq2, capturedUUID2 := capturedReq, capturedUUID
							go func() {
								ctx := context.Background()
								aiResp, err := aiHandler.HandleAIMessage(ctx, &capturedReq2)
								if err != nil {
									zlog.Error("AI fallback goroutine error: " + err.Error())
									return
								}
								jsonMsg, _ := json.Marshal(aiResp)
								mb := &MessageBack{Message: jsonMsg, Uuid: capturedUUID2}
								s.mutex.Lock()
								if cli, ok := s.Clients[capturedReq2.SendId]; ok {
									cli.SendBack <- mb
								}
								s.mutex.Unlock()
							}()
					...
					if message.ReceiveId[0] == 'U' { // 发送给User
						...
						messageRsp := respond.GetMessageListRespond{
							...
							ClientSeq:  chatMessageReq.ClientSeq,
							ServerSeq:  chatMessageReq.ServerSeq,
							TraceId:    chatMessageReq.TraceId,
							Type:       message.Type,

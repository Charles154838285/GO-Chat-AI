...
func (g *groupInfoService) LoadMyGroup(ownerId string) (string, []respond.LoadMyGroupRespond, int) {
	rspString, err := myredis.GetKeyNilIsErr("contact_mygroup_list_" + ownerId) // 从redis中获取我的群聊列表
	if err != nil {
		if errors.Is(err, redis.Nil) { // 缓存中没有我的群聊列表
			var groupList []model.GroupInfo
			if res := dao.GormDB.Order("created_at DESC").Where("owner_id = ?", ownerId).Find(&groupList); res.Error != nil { // 从数据库中查询我的群聊列表
				zlog.Error(res.Error.Error())
